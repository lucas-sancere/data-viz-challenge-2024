...
with tab2:
    ...
    if st.sidebar.button("Get Data"):
        with st.spinner('Downloading data...'):
            ...
            request = {
                "variable": [ variable_options_dico[selected_variable] ],
                "model": ["ensemble"],
                "level": ["0"],
                "date": [date],
                "type": ["forecast"],
            ...
            with st.spinner('Extracting data...'):
                ...
                with zip_file.open(netcdf_filename) as nc_file:
                    
                    ds = xr.open_dataset(io.BytesIO(nc_file.read()))
                    ...
                    def get_available_hours(selected_data):
                        ...
                        # Convert times to total hours using pandas
                        available_hours = pd.to_timedelta(times).total_seconds() / 3600  # Convert seconds to hours
                        hours_options = np.unique(available_hours)
                        hours_options.sort()
                        return times, hours_options
                    ...
                    def generate_map(selected_data, selected_hour, mode):
                        ...
                        # Convert times to total hours
                        times_in_hours = pd.to_timedelta(times).total_seconds() / 3600
                    
                        # Find the index where times match the selected_hour
                        time_index = np.argmin(np.abs(times_in_hours - selected_hour))
                        selected_time = dataset.time.values[time_index]
                    
                        # Now select the data at this time
                        data = dataset.sel(time=selected_time, level=0.0)
                        ...
                        aerosol_interp = aerosol.interp(latitude=new_lat, longitude=new_lon, method='linear')
